defmodule GS.Master do
  ...
  def handle_call({:process, opts}, _from, state) do
    ...
    Logger.debug(
      "command line_arg nodes - #{inspect(num_nodes)}  topology -  #{inspect(topology)} algorithm - #{
        inspect(algorithm)
      } "
    )
        ...
        fn x ->
          {:ok, agent_node_pid} =
            DynamicSupervisor.start_child(
              GS.NodeSupervisor,
              {GS.NodeGen, [x, algorithm, topology]}
            )
    ...
    GenServer.cast(GS.Node_failure_instrumentor, %{
      master_pid: master_pid,
      num_nodes: num_kill_nodes,
      type: num_kill_type,
  ...
  defp wait_for_convergence(last_timestamp) do
    ...
    after
      5_000 ->
        ...
        Logger.info(
          "Timestamps: last- #{inspect(last_timestamp)}  beacon- #{inspect(last_beacon_ts)}"
        )
  ...
  defp get_beacon_timestamp() do
    Logger.debug(
      "#{inspect(__MODULE__)} get beacon ts #{inspect(Registry.lookup(GS.Registry.Beacon, :c))}"
    )
